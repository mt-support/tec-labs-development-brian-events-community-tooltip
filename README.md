...
* `development-brian-events-community-tooltip` - Plugin Human-readable name, e.g. "Sample Extension"
* `Developmentbrianeventscommunitytooltip` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `development-brian-events-community-tooltip` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `development_brian_events_community_tooltip` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
